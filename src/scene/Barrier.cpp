...
int Barrier::checkCollisionDirection(sf::FloatRect playerRect, sf::Vector2f speed) {
    for(const auto& barrier : barrierRects){
        if (playerRect.intersects(barrier)){
            ...
            //如果距离很近且在下落，视为下方碰撞
            if(kirbyBottom-barrier.top >=0 && kirbyBottom-barrier.top <= 6.0 && speed.y > 0){
                return 1; //可站立移动
            }
            if(playerRect.top-barrierBottom >=0 && playerRect.top-barrierBottom <= 6.0 && speed.y < 0){
                return 3; //卡比头撞到障碍物
            }
            if(kirbyRight-barrier.left >=0 && kirbyRight-barrier.left <= 6.0 && speed.x > 0)return 4; // 卡比向右走碰撞障碍物
            if(barrierRight-playerRect.left >=0 && barrierRight-playerRect.left <= 6.0 && speed.x < 0)return 2; // 卡比向左走碰撞障碍物
        }
    }
    return 0;
}